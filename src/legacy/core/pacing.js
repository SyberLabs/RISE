...
export class StateCurve {
     ...
     */
    static induction() {
        return new StateCurve(p => 1.0 + p * 0.5);
    }
     ...
     */
    static ascent() {
        return new StateCurve(p => 1.5 - p * 0.5);
    }
    ...
    static wave(frequency = 3.0) {
        return new StateCurve(p => 1.0 + 0.3 * Math.sin(p * frequency * 2 * Math.PI));
    }
    ...
    static climax(peakPosition = 0.75) {
        return new StateCurve(p => {
            if (p < peakPosition) {
                ...
                return 1.0 - 0.3 * (p / peakPosition);
            } else {
                // Decelerate after peak
                return 0.7 + 0.5 * ((p - peakPosition) / (1 - peakPosition));
            }
        });
    }
