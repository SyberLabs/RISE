...
export class AudioScheduleController {
    ...
    get generation() { return this._generation; }
    get activeCueId() { return this._activeCueId; }

    /** A reader may silence a Journey without rewriting it (§3.3). */
    setEnabled(enabled) {
        this._enabled = enabled !== false;
        if (!this._enabled) this.silence();
    }

    observe(atom) {
        if (!this.program || !this._enabled) return null;
        ...
        if (id === this._activeCueId) return { id, cue };

        this._activeCueId = id;
        this._generation += 1;
        this._apply(cue, this._generation);
        return { id, cue };
    }
