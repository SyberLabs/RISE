 ...
 * browsing axis. Each category's pool is these pinned works, drawn
 ...
 *
 * THE ART INSTITUTE IS NO LONGER AMONG THEM (swept 2026-09-17).
 *
 * Every AIC work builds its image on www.artic.edu/iiif, which answers a
 * cross-origin <img> with Cf-Mitigated: challenge and
 * Cross-Origin-Resource-Policy: same-origin. An <img> cannot solve a
 * JavaScript challenge, so Chromium refuses it as
 * ERR_BLOCKED_BY_RESPONSE.NotSameOrigin before a pixel is decoded.
 *
 * Measured in a browser rather than inferred: all 460 unique AIC ids
 * pinned here resolved their metadata through api.artic.edu, and 0 of 460
 * images loaded. The same run sampled what would remain — 40 Rijksmuseum
 * and 40 Cleveland — and found 76 of 80 serving with no blocked image at
 * all, the other 4 missing at the metadata step, which is ordinary
 * attrition and not a wall.
 *
 * 571 pin slots went, 719 remain. The curation stands in the history; it
 * was the delivery that failed, and no amount of keeping dead ids here
 * makes a picture arrive. Anything that needs those works again needs
 * them re-pinned from a museum that serves.
 *
...
export const MUSEUM_CATEGORY_PINS = Object.freeze({
    ...
    // UKIYO-E IS GONE RATHER THAN EMPTY.
    //
    // It was withdrawn in #157 because every one of its 100 works was
    // the Art Institute's, and its pins were kept on the reasoning that
    // restoring the category would then be deleting one line. That is no
    // longer true: all 100 were measured blocked, so restoring it would
    // offer a category with nothing in it. The curation is in the history
    // and bringing it back now means re-pinning it from a museum that
    // still serves, not un-commenting a name.
    "flowers": [
